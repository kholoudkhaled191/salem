...
<title>Edit Stock #{{ $stock->id }}</title>
...
<div class="main">
  <div class="section-card">
    <h3><i class="fas fa-clipboard-list"></i> Edit Stock #{{ $stock->id }}</h3>
    ...
    <form method="POST" action="{{ route('stocks.update', $stock->id) }}">
        ...
        <input type="date" name="storage_date" value="{{ $stock->storage_date }}" required>

        <label>Meters Quantity:</label>
        <input type="number" step="0.01" name="meters_quantity" value="{{ $stock->meters_quantity }}">

        <label>Rolls Quantity:</label>
        <input type="number" name="rolls_quantity" value="{{ $stock->rolls_quantity }}">
