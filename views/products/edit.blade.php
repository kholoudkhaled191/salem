...
<title>Edit Product | Salem Dashboard</title>
...
#colors-wrapper input, #images-wrapper input{width:calc(100% - 0px);margin-bottom:5px;}
    ...
    <form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="price-wrapper">
            <div class="form-group">
                ...
                <input type="number" step="0.01" name="price_before" value="{{ old('price_before', $product->price_before) }}" required>
            </div>
            <div class="form-group">
                <label>Price After:</label>
                <input type="number" step="0.01" name="price_after" value="{{ old('price_after', $product->price_after) }}" required>
            </div>
        </div>
        ...
        <div class="form-group">
            <label>Colors:</label>
            @php
                $colors = is_array($product->colors) ? $product->colors : explode(',', $product->colors ?? '');
            @endphp
            <div id="colors-wrapper">
                @foreach($colors as $color)
                    <input type="text" name="colors[]" value="{{ $color }}" placeholder="Enter color"><br>
                @endforeach
            </div>
        ...
        <div class="form-group">
            ...
            <div id="images-wrapper">
                @foreach($images as $img)
                    <input type="text" name="images[]" value="{{ $img }}" placeholder="Enter image URL"><br>
                @endforeach
            </div>
            <button type="button" class="btn-add" onclick="addImage()"><i class="fas fa-plus"></i> Add Image</button>
        </div>
