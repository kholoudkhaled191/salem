...
.section-card h1{font-size:22px;font-weight:600;margin-bottom:20px;color:var(--primary);display:flex;align-items:center;gap:10px;}
.btn-add{display:inline-block;margin-bottom:20px;padding:10px 20px;background:var(--accent);color:#fff;border-radius:8px;text-decoration:none;font-weight:600;transition:0.3s;}
.btn-add:hover{background:#b5935f;}
.table{width:100%;border-collapse:collapse;margin-top:10px;}
.table th,.table td{padding:12px 15px;border:1px solid #ddd;text-align:left;}
.table th{background:#f5f5f5;}
.btn-sm{padding:6px 12px;border-radius:6px;font-size:14px;font-weight:500;cursor:pointer;border:none;transition:0.3s;}
.btn-warning{background:#f0ad4e;color:#fff;}
.btn-warning:hover{background:#d6953a;}
.btn-danger{background:#e74c3c;color:#fff;}
.btn-danger:hover{background:#c0392b;}
.alert-success{padding:10px 15px;background:#d1e7dd;color:#0f5132;border-radius:6px;margin-bottom:20px;}
</style>
</head>
<body>
...
<div class="sidebar">
  ...
  <ul class="sidebar-menu">
    ...
    <li><a href="{{ route('offer_products.index') }}" class="active"><i class="fas fa-tags"></i> Offer Products</a></li>
...
<div class="main">
  <div class="section-card">
    <h1><i class="fas fa-tags"></i> Offer-Products List</h1>
    <a href="{{ route('offer_products.create') }}" class="btn-add"><i class="fas fa-plus"></i> Add New Offer-Product</a>
    ...
    <table class="table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <a href="{{ route('offer_products.edit', $op->id) }}" class="btn-sm btn-warning"><i class="fas fa-edit"></i> Edit</a>
                    <form action="{{ route('offer_products.destroy', $op->id) }}" method="POST" style="display:inline-block;">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn-sm btn-danger" onclick="return confirm('Are you sure?')"><i class="fas fa-trash-alt"></i> Delete</button>
                    </form>
                </td>
            </tr>
            @endforeach
