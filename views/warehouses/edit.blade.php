...
input{padding:8px;border-radius:6px;border:1px solid #ccc;width:100%;margin-bottom:10px;}
...
a.back-link{display:inline-block;margin-top:15px;color:#C6A972;text-decoration:none;}
a.back-link:hover{color:#B5925E;}
.alert-danger{background:#f8d7da;color:#721c24;padding:10px 15px;border-radius:6px;margin-bottom:15px;}
</style>
</head>
<body>
...
<div class="sidebar">
  ...
  <ul class="sidebar-menu">
    ...
    <li><a href="{{ route('warehouses.index') }}" class="active"><i class="fas fa-warehouse"></i> Warehouses</a></li>
...
<div class="main">
  <div class="section-card">
    <h3><i class="fas fa-warehouse"></i> Edit Warehouse #{{ $warehouse->id }}</h3>
    ...
    <form action="{{ route('warehouses.update', $warehouse->id) }}" method="POST">
        ...
        <input type="text" name="name" value="{{ $warehouse->name }}" placeholder="Warehouse Name" required>
        <input type="number" name="branch_id" value="{{ $warehouse->branch_id }}" placeholder="Branch ID" required>
        <input type="text" name="location" value="{{ $warehouse->location }}" placeholder="Location" required>
        <button type="submit">Update</button>
    </form>

    <a href="{{ route('warehouses.index') }}" class="back-link">⬅ Back to Warehouse List</a>
  </div>
</div>
