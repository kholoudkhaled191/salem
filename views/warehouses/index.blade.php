...
button, .btn{padding:6px 12px;border-radius:6px;border:none;cursor:pointer;}
.btn-add{background:var(--accent);color:#fff;margin-bottom:15px;text-decoration:none;display:inline-block;}
.btn-add:hover{background:#B5925E;}
.btn-edit{background:#f39c12;color:#fff;margin-right:5px;}
.btn-edit:hover{background:#d78b0b;}
...
<div class="main">
  <div class="section-card">
    <h3><i class="fas fa-warehouse"></i> Warehouses List</h3>

    @if (session('success'))
        <div class="alert-success">{{ session('success') }}</div>
    @endif

    <a href="{{ route('warehouses.create') }}" class="btn-add">+ Add Warehouse</a>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
                <a href="{{ route('warehouses.edit', $warehouse->id) }}" class="btn-edit">Edit</a>
                <form action="{{ route('warehouses.destroy', $warehouse->id) }}" method="POST" class="inline-form">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn-delete" onclick="return confirm('Delete this warehouse?')">Delete</button>
                </form>
            </td>
          </tr>
          @empty
